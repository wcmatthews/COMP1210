...
public class ConeList {
   ...
   public Cone findCone(String labelIn, ArrayList<Cone> cList) {
      for (Cone c : cList) {
         if(c.getLabel().equals(labelIn.toLowerCase())) {
            return c;
         }
      }
      return null;
   ...
   public boolean deleteCone(String labelIn, double heightIn, double radiusIn) {
      
      boolean result = false;
      int index = -1;
      for (Cone c : cList) {
         if(c.getLabel() == labelIn
            && c.getHeight() == heightIn
            && c.getRadius() == radiusIn) {
